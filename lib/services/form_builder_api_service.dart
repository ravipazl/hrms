...
class FormBuilderAPIService {
  ...
  static const String baseUrl = '${ApiConfig.djangoBaseUrl}/form-builder/';
  ...
  Future<String> submitForm(
    ...
    try {
      ...
      cleanedFormData.forEach((key, value) {
        print(
          '  ✅ $key: ${value.runtimeType} = ${value is String && value.length > 50 ? "${value.substring(0, 50)}..." : value}',
        );
      });
  ...
  Map<String, dynamic> _cleanFormData(Map<String, dynamic> formData) {
    ...
    formData.forEach((key, value) {
      if (value == null) {
        ...
        final hasInlineFields = map.keys.any(
          (k) => k.toString().startsWith('field_'),
        );
        ...
        if (hasInlineFields && !hasContent) {
          ...
          print(
            '  🔧 FIXING rich text field "$key": building HTML from inline data (OLD FORMAT)',
          );

          final buffer = StringBuffer('<p>');
          var hasValues = false;
          map.forEach((fieldId, fieldValue) {
            if (fieldId.toString().startsWith('field_') &&
                fieldValue != null &&
          ...
          print(
            '  🔧 Rich text field "$key": KEEPING complete object with embeddedFieldValues',
          );
  ...
  Future<String?> uploadSignature(Uint8List pngBytes, String fieldId) async {
    try {
      ...
      final formData = FormData.fromMap({
        'file': MultipartFile.fromBytes(pngBytes, filename: 'signature.png'),
        'field_id': fieldId,
      });
