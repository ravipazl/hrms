...
class WorkflowExecutionApiService {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        ...
        onError: (error, handler) {
          print(
            '❌ Workflow Execution API Error: ${error.response?.statusCode} - ${error.message}',
          );
  ...
  Future<Map<String, dynamic>> getWorkflowExecutionStatus(
    ...
    try {
      print(
        '🔍 Loading workflow execution status for requisition: $requisitionId',
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          print(
            '📝 Templates in results field with ${data['results'].length} items',
          );
